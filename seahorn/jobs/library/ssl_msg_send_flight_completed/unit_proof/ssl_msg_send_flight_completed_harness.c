...
DEFINE_UNIT_PROOF(mbedtls_ssl_send_flight_completed) {
  ...
  sassert(ssl.handshake->retransmit_state == MBEDTLS_SSL_RETRANS_FINISHED ||
          ssl.handshake->retransmit_state == MBEDTLS_SSL_RETRANS_WAITING);
}

int main(void) {
  CALL_UNIT_PROOF(mbedtls_ssl_send_flight_completed);
  return 0;
}
