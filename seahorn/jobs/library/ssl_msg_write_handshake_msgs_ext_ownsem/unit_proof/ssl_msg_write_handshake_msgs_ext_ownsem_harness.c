...
DEFINE_UNIT_PROOF(mbedtls_ssl_write_handshake_msg_ext) {
  ...
  for(int k=0; k < NUM_SSL_CTX; k++) {
    ...
    unsigned char* buf = bufs[idx].out_buf;
    SEA_MKOWN(buf);
    SEA_WRITE_CACHE(ssl, bufs[idx].out_buf_len);
    SEA_BORROW(ssl->out_buf, buf);
    ...
    //size_t seq_number = get_sequence_number(ssl);
    size_t idx_count;
    //SEA_READ_CACHE(idx_count, ssl->out_buf);
    //SEA_WRITE_CACHE(ssl, seq_number + idx_count);
    //SEA_WRITE_CACHE(ssl->out_buf, seq_number + idx_count);
    //sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *) ssl, seq_number + idx_count);
    //sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *) ssl->out_buf, seq_number + idx_count);
    //sea_printf("Counter initial value:%ld\n", sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *) (&ssl)));
    mbedtls_ssl_context *bor_ssl;
    SEA_BORROW(bor_ssl, ssl)
    // NOTE: call the SUT
    int rc = mbedtls_ssl_write_handshake_msg_ext(bor_ssl, checksum, 1);
    ...
    //   size_t counter;
    //   SEA_READ_CACHE(counter, ssl); 
    //   sassert(counter == seq_number + 1 + idx_count);
    // }        
    // if (rc == 0 && append_flight) {
    //   size_t cpysrc;
    //   SEA_READ_CACHE(cpysrc, ssl);
    //   // size_t j = nd_size_t();
    //   // assume(j < NUM_OUT_BUFS);
    //   // assume (j != idx);
...
/*   if (results[m].rc == 0 && results[m].append_flight && results[n].rc == 0 && results[n].append_flight) {
    size_t cpysrc;
    SEA_READ_CACHE(cpysrc, results[m].ssl);
    size_t src; 
    SEA_READ_CACHE(src, results[n].buf);
    sassert(cpysrc != src);
  } */
  size_t p = nd_size_t();
  ...
  if(results[p].hs_type != MBEDTLS_SSL_HS_HELLO_REQUEST && results[p].rc != MBEDTLS_ERR_SSL_BAD_INPUT_DATA) {
    //sassert(sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *) results[p].buf) == 1);
    size_t out_msglen;
    SEA_READ_CACHE(out_msglen, results[p].ssl);
    sassert(out_msglen > 10);
  }
