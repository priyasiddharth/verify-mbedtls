...
extern void *objcopy(void *restrict dst, const void *restrict src, size_t n) {
  sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *) src, 
        sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *) src) + 1);
  sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *) dst, 
        sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *) dst) + 1);      
...
static int ssl_flight_append(mbedtls_ssl_context *ssl)
{
    ...
    objcopy(msg->p, ssl->out_msg, ssl->out_msglen);
    ...
    if (ssl->handshake->flight == NULL) {
        ...
        sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *) ssl->out_buf, sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *) ssl->out_buf) + 1);
    } else {
...
{
    int ret = MBEDTLS_ERR_ERROR_CORRUPTION_DETECTED;
    size_t out_msg_len = USE_SHADOW ? sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *)ssl) : ssl->out_msglen;
     ...
     */
    out_msg_len = USE_SHADOW ? sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *)ssl) : ssl->out_msglen; 
    ...
    if (ssl->out_msgtype == MBEDTLS_SSL_MSG_HANDSHAKE) {
        ...
        if (ssl->conf->transport == MBEDTLS_SSL_TRANSPORT_DATAGRAM) {
            ...
            out_msg_len = sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *)ssl);
            if (MBEDTLS_SSL_OUT_CONTENT_LEN - out_msg_len /* ssl->out_msglen */ < 8) {
            ...
            out_msg_len = USE_SHADOW ? sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *)ssl) : ssl->out_msglen;
            #if USE_SHADOW == 1 
            sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *)ssl, out_msg_len + 8);
            #else 
            ssl->out_msglen += 8;
            #endif 
...
int mbedtls_ssl_write_record(mbedtls_ssl_context *ssl, int force_flush)
{
    ...
    if (!done) {
        ...
        if (ssl->transform_out != NULL) {
            ...
            if ((ret = mbedtls_ssl_encrypt_buf(fk_bor_ssl, fk_bor_ssl->transform_out, &rec,
                                              fk_bor_ssl->conf->f_rng, fk_bor_ssl->conf->p_rng)) != 0) {
        ...
        //     }
        // }
        size_t count = sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *) ssl->out_buf) + 1;
        sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *) ssl, count);
        sea_set_shadowmem(TRACK_CUSTOM0_MEM, (char *) ssl->out_buf, count);
        sea_printf("counter(post-inc):%ld\n",sea_get_shadowmem(TRACK_CUSTOM0_MEM, (char *)ssl));
        if (count == 0) return MBEDTLS_ERR_SSL_COUNTER_WRAPPING; 
    ...
    if ((flush == SSL_FORCE_FLUSH) &&
        (ret = mbedtls_ssl_flush_output(fk_bor_ssl)) != 0) {
